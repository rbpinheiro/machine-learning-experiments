...
exports.default = [[1, 2], [2, 1], [2, 4], [1, 3], [2, 2], [3, 1], [1, 1], [7, 3], [8, 2], [6, 4], [7, 4], [8, 1], [9, 2], [10, 8], [9, 10], [7, 8], [7, 9], [8, 11], [9, 9]];
...
var Data = {
  getRanges: function getRanges(extremes) {
    return extremes.map(function (extreme) {
      return extreme.max - extreme.min;
    });
  },
  getExtremes: function getExtremes(data) {
    return data.reduce(function (result, row) {
      row.forEach(function (dimension, index) {
...
var Mean = {
  init: function init(k, data) {
    var extremes = Data.getExtremes(data);
    var ranges = Data.getRanges(extremes);

    return [].concat((0, _toConsumableArray3.default)(Array(k).keys())).map(function () {
      return extremes.map(function (extreme, index) {
        return extreme.min + Math.random() * ranges[index];
      });
    });
  },
  makeAssignments: function makeAssignments(means, data) {
    return data.map(function (row) {
      var distances = means.map(function (mean) {
        return Math.sqrt(row.reduce(function (total, currentDimension, index) {
          return total + currentDimension - mean[index];
        }, 0));
      });
  ...
  moveMeans: function moveMeans(means, data) {
    var assignments = this.makeAssignments(means, data);

    var initialData = means.reduce(function (result, mean) {
      result.counts.push(0);
      result.sums.push(mean.map(function () {
        return 0;
      }));
      return result;
    }, {
    ...
    var calculatedData = assignments.reduce(function (result, meanIndex, rowIndex) {
      var row = data[rowIndex];
      var mean = means[meanIndex];

      result.counts[meanIndex] += 1;

      mean.forEach(function (dimension) {
...
var means = Mean.init(3, _data2.default);
